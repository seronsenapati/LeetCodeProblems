...
## Search Problem
<input type="text" id="problemSearch" onkeyup="searchProblem()" placeholder="Enter problem number or title..." style="width: 100%; padding: 10px; margin: 10px 0; border: 1px solid #ccc; border-radius: 4px;">
...
function searchProblem() {
  ...
  for (i = 0; i < tables.length; i++) {
    ...
    for (j = 1; j < tr.length; j++) {
      ...
      found = false;
      if (td) {
        // Check Problem Number (column 0) and Title (column 1)
        for (let k = 0; k < 2; k++) {
          if (td[k]) {
